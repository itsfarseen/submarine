...
func DecodeEvents(metadata *v12.Metadata, eventBytes []byte) ([]EventRecord, error) {
...
func DecodeEventRecord(metadata *v12.Metadata, r *Reader) (EventRecord, error) {
...
func DecodePalletEvent(metadata *v12.Metadata, r *Reader) (*DecodedPalletVariant, error) {
	...
	// In metadata v12 and older, the pallet index is the index in the filtered
	// list of pallets that actually have events.
	eventfulModules := make([]v12.ModuleMetadata, 0)
	for _, p := range metadata.Modules {
		if p.Events.HasValue {
			eventfulModules = append(eventfulModules, v12.ModuleMetadata(p))
		}
	}
