...
type MapType struct {
	...
	Key    Text // Type in v13.ts
	Value  Text // Type in v13.ts
	Linked bool
}

type DoubleMapType struct {
	Hasher     StorageHasher
	Key1       Text // Type in v13.ts
	Key2       Text // Type in v13.ts
	Value      Text // Type in v13.ts
	Key2Hasher StorageHasher
}

type NMapType struct {
	KeyVec  []Text // Vec<Type> in v13.ts
...
func DecodeMetadata(r *Reader) (Metadata, error) {
	...
	result.Extrinsic, err = DecodeExtrinsicMetadataV13(r)
	if err != nil {
...
func DecodeStorageEntryType(r *Reader) (StorageEntryType, error) {
	...
	switch result.Kind {
	case KindPlain:
		result.Plain, err = DecodeText(r) // 'Type' is Text
		if err != nil {
			return result, err
		}
	case KindMap:
	...
	case KindNMap:
		result.NMap.KeyVec, err = DecodeVec(r, DecodeText)
		if err != nil {
			return result, err
		}

		result.NMap.Hashers, err = DecodeVec(r, func(r *Reader) (StorageHasher, error) {
			hasher, err := DecodeU8(r)
			return StorageHasher(hasher), err
		})
		if err != nil {
			return result, err
		}

		result.NMap.Value, err = DecodeText(r)
		if err != nil {
			return result, err
		}
	default:
...
func DecodeExtrinsicMetadataV13(r *Reader) (ExtrinsicMetadataV13, error) {
	var result ExtrinsicMetadataV13
	var err error
