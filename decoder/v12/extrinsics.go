...
func DecodeExtrinsic(metadata *v12.Metadata, extrinsicBytes []byte) (*DecodedExtrinsic, error) {
	...
	if isSigned {
		...
		// In v12, signed extensions are just a Vec<Text>, where each Text is a type name.
...
func DecodeCall(metadata *v12.Metadata, r *Reader) (*DecodedPalletVariant, error) {
	...
	callableModules := make([]v12.ModuleMetadata, 0)
